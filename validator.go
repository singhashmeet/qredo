...
func validateValue(s string) (string, error) {
	...
	number, tail, err := validateNumber(s)
	if err != nil {
		return tail, fmt.Errorf("cannot parse number: %s", err)
	}
	globalNumbers = append(globalNumbers, string(number))
	return tail, nil
}
...
func validateNumber(s string) ([]byte, string, error) {
	var number []byte
	if len(s) == 0 {
		return number, s, fmt.Errorf("zero-length number")
	}
	if s[0] == '-' {
		number = append(number, '-')
		s = s[1:]
		if len(s) == 0 {
			return number, s, fmt.Errorf("missing number after minus")
		}
	}
	i := 0
	for i < len(s) {
	...
	if i <= 0 {
		return number, s, fmt.Errorf("expecting 0..9 digit, got %c", s[0])
	}
	if s[0] == '0' && i != 1 {
		return number, s, fmt.Errorf("unexpected number starting from 0")
	}
	if i >= len(s) {
		return number, "", nil
	}
	if s[i] == '.' {
		...
		s = s[i+1:]
		if len(s) == 0 {
			return number, s, fmt.Errorf("missing fractional part")
		}
		i = 0
		for i < len(s) {
			if s[i] < '0' || s[i] > '9' {
		...
		if i == 0 {
			return number, s, fmt.Errorf("expecting 0..9 digit in fractional part, got %c", s[0])
		}
		if i >= len(s) {
			return number, "", nil
		}
	...
	if s[i] == 'e' || s[i] == 'E' {
		...
		s = s[i+1:]
		if len(s) == 0 {
			return number, s, fmt.Errorf("missing exponent part")
		}
		if s[0] == '-' || s[0] == '+' {
			s = s[1:]
			if len(s) == 0 {
		...
		if i == 0 {
			return number, s, fmt.Errorf("expecting 0..9 digit in exponent part, got %c", s[0])
		}
		if i >= len(s) {
			return number, "", nil
		}
	}
	return number, s[i:], nil
}
